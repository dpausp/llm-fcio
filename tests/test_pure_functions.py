...
class TestChunkLinesEmptyInput:
...
class TestChunkLinesExactFit:
    ...
    def test_exact_fit_two_chunks_no_overlap(self) -> None:
    ...
    def test_exact_fit_two_chunks_content(self) -> None:
        text = "\n".join(f"line{i}" for i in range(10))
        result = _chunk_lines(text, "f.py", 5, 0)
...
class TestChunkLinesOverlap:
    def test_overlap_produces_three_chunks(self) -> None:
        text = "\n".join(f"line{i}" for i in range(10))
    ...
    def test_overlap_equal_to_chunk_size_infinite_loop(self) -> None:
...
class TestChunkLinesChunkIds:
    def test_chunk_ids_are_one_based(self) -> None:
        result = _chunk_lines("a\nb\nc", "app.py", 2, 0)
        # First chunk starts at line 1
        assert result[0][0].startswith("app.py:1-")

    def test_filepath_in_chunk_id(self) -> None:
        result = _chunk_lines("a\nb", "src/main.py", 2, 0)
...
class TestDiscoverFilesExistingFiles:
    ...
    def test_file_not_matching_glob_skipped(self, tmp_path: Path) -> None:
        f = tmp_path / "code.py"
        f.write_text("code")
...
class TestDiscoverFilesNonexistentPaths:
    ...
    def test_nonexistent_path_in_message(self, tmp_path: Path) -> None:
...
class TestDiscoverFilesHardExcludes:
    ...
    def test_dunder_pycache_excluded(self, tmp_path: Path) -> None:
...
class TestDiscoverFilesEmptyDirectory:
...
class TestBuildChatBodyWithMaxTokens:
    def test_includes_max_tokens(self) -> None:
        body = _build_chat_body("gpt-4", [{"role": "user", "content": "hi"}], 0.7, 100)
        assert body["max_tokens"] == 100

    def test_dict_structure_with_max_tokens(self) -> None:
...
class TestBuildChatBodyWithoutMaxTokens:
    def test_omits_max_tokens_when_none(self) -> None:
        body = _build_chat_body("gpt-4", [{"role": "user", "content": "hi"}], 0.7, None)
        assert "max_tokens" not in body

    def test_dict_structure_without_max_tokens(self) -> None:
        msgs = [{"role": "user", "content": "hello"}]
...
class TestBuildChatBodyEdgeCases:
    ...
    def test_max_tokens_zero_is_truthy(self) -> None:
